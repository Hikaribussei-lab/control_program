...
def initializePiCamera():
    global cam, shutter_speed, iso, count, pinnum, step
    if count == 0:
...
if __name__ == '__main__':
    ...
    while True:
        ...
        if command[0:2] == 'CG':# move tage to home
            step = int(command[2:])
                    ...
                    with connection:
                        while not quitFlag:
                            ...
                            if command == "measure":
                                ...
                                print(cam.analog_gain, cam.digital_gain, cam.exposure_speed, cam.iso, cam.step)
                                connection.sendall(b'OK')
