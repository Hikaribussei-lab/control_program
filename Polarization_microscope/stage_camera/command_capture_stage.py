...
if __name__ == '__main__':
    ...
    while True:
        ...
        if command == 'S':
            ...
            print(filename)
        if command == 'V':
            ser = serial.Serial(COMV, timeout=0.1)
            ser.write(b"Z, F1, PR2\r\n")
            time.sleep(0.5)
            ser.read_all()
            vl_list=[]
            for num in range(10):
                ...
                time.sleep(0.5)
                data=ser.read_all()
                vl=float(data.split()[2])
                #print(vl)
                vl_list.append(vl)
                #vl=data.replace("MD?\r\nDV", "").replace("\r\n\n=>\r\n", "")
            vl_av=sum(vl_list)/len(vl_list)
            vl_def=sum((x-sum(vl_list)/len(vl_list))**2  for x in vl_list)/len(vl_list)
            print(vl_av, vl_def)
            ser.close()
        if command == 'W':
                ...
                while not quitFlag:
                    ...
                    with connection:
                        while not quitFlag:
                            ...
                            if command == "measure":
                                array = takeImage()
                                filename = save_directory + datetime.now().strftime("Img_%Y%m%d_%H%M%S.png")
                                img = Image.fromarray((array / 4).astype(np.uint8))
                                img.save(filename)
                                ...
                                vl_av=sum(vl_list)/len(vl_list)
                                vl_def=sum((x-sum(vl_list)/len(vl_list))**2  for x in vl_list)/len(vl_list)
                                connection.sendall(str(vl_av).encode())
                                connection.sendall(str(vl_def).encode())
                                ser.close()
                            elif command == "send":
                                if filename is None:
