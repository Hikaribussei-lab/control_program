...
FILENAME = f"{dt.now():%Y%m%d%H%M%S}.tif" #'image#.tif'  # The filename of the TIFF
...
with TLCameraSDK() as sdk:
    ...
    with sdk.open_camera(cameras[0]) as camera:
        ...
        while frames_counted < NUMBER_OF_IMAGES:
            ...
            with tifffile.TiffWriter(OUTPUT_DIRECTORY + os.sep + FILENAME, append=True
                                     ) as tiff:
                """
                ...
                tiff.save(data=image_data # np.ushort image data array from the camera
                          #compress="lzma"   # amount of compression (0-9), by default it is uncompressed (0)
...
with tifffile.TiffFile(OUTPUT_DIRECTORY + os.sep + FILENAME) as tiff_read:
    ...
    #print("First Image: Bit Depth = {} bpp, Exposure Time = {} ms".format(page_one.tags[str(TAG_BITDEPTH)].value, page_one.tags[str(TAG_EXPOSURE)].value/1000))
